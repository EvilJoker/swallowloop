...
**优先融合，再考虑新增**：新功能先考虑融入已有模块，职责不符才考虑新建。
...
# ❌ 错误：直接依赖具体实现
class ExecutorService:
    def __init__(self, agent: DeerFlowAgent):
        self._agent = agent
```
...
2. **具体实现注入**
   ```python
   # ❌ 禁止
   service = ExecutorService(agent=DeerFlowAgent())
...
2. **是否需要跨模块联动？**
   - 新功能涉及多个模块的协作
   - 在调用方（Application/Interface 层）组合
   - 如 Issue 创建 → IssueService 组合多个 Domain 模块
...
|------|------|
| 功能是某模块职责的自然延伸 | 融入该模块 |
| 功能需要多个模块协作 | 调用方组合，模块保持独立 |
| 功能与所有模块都无关 | 新建模块 |

**反面案例**：
- 为了"清晰"把 `sanitize_log_message` 单独建 `logging_utils.py` → 应该融入 `logger` 模块

**正面案例**：
- `sanitize_log_message` 依赖 `SENSITIVE_KEYS` 常量 → 合并到 `logger/__init__.py`
