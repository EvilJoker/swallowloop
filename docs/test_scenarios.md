...
```bash
# 查看配置
cat .env | grep GITHUB_REPOS

# 查看日志
grep "SourceControl" ~/.swallowloop/logs/swallowloop.log
```
...
**场景描述**: 不同仓库的任务使用独立的分支和工作空间。

**前置条件**:
- 配置多个仓库

**测试步骤**:
1. 在 repo1 创建一个 Issue
2. 在 repo2 创建一个 Issue

**预期结果**:
- repo1 的任务分支名: `repo1_feature_1_0318`
- repo2 的任务分支名: `repo2_feature_2_0318`
- 工作空间目录: `~/.swallowloop/workspaces/repo1_feature_1_0318/`
- PR 提交到对应仓库
...
**预期结果**:
- 日志显示每个仓库的扫描结果
- 如:「仓库 owner/repo1: 发现 0 个新任务, 0 个待中止」
- 任务列表包含来自不同仓库的任务
