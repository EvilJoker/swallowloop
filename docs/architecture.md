...
支持多种 LLM 提供商，Agent 可从配置中获取认证信息：

| 提供商 | 环境变量 | 说明 |
|-------|---------|------|
| iFlow (默认) | - | 使用本地 iFlow CLI 默认配置 |
| OpenAI | `LLM_PROVIDER=openai` | OpenAI API |
| MiniMax | `LLM_PROVIDER=minimax` | MiniMax API |
| 自定义 | `LLM_PROVIDER=custom` | 支持任意 OpenAI 兼容 API |

**配置环境变量**:
- `LLM_PROVIDER`: 提供商 (iflow/openai/minimax/custom)
- `LLM_API_KEY`: API Key
...
1. **本地工作空间**：删除任务对应的本地代码目录 (`~/.swallowloop/workspaces/`)
2. **远端分支**：删除 GitHub 上的对应分支

清理在任务状态变为 `completed` 时自动触发。
...
- REST API：任务列表、详情、统计
- WebSocket：实时日志推送
- **进程间数据同步**：每次 API 请求自动重新加载文件，确保获取最新数据
- 默认端口：8080
